...
const StaffDetails = () => {
  ...
  const fetchStaff = () => {
    staffService.getStaffById(id).then((data) => {
      console.log(data);
      setStaffData(data);
      setLoading(false);
    });
  ...
  const handleHire = () => {
    sessionStorage.setItem("order-item", JSON.stringify(staffData));
    history.push("/app/staffcheckout");
  };
      ...
      <div className="col-md-10 mx-auto">
        ...
        <div className="row">
          <div className="col-md-6">
            <img
              src={`${url}${staffData.avatar}`}
              className={clsx(cssClasses.image, "img-fluid")}
            />
          </div>
          <div className="col-md-6">
            ...
            <Button variant="contained" className="mt-5" onClick={handleHire}>
              Hire Now
            </Button>
          </div>
        </div>
  ...
  else return <h1>Nothing Here</h1>;
};
